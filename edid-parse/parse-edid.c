...
int ParseEdidBlock0(unsigned char *i_pEdid)
{
	int i = 0;
    int j = 0;
    unsigned char *block;
...
int string_to_hex( const char *buf)
{
	unsigned char ret = 0;
	ret = (buf[0] >= '0' && buf[0] <= '9') ? ((buf[0] - '0') << 4) : 
			( (buf[0] >= 'a' && buf[0] <= 'f') ? ((buf[0] - 'W') << 4) : 
			 ((buf[0] - '7') << 4) );
			
	ret |= (buf[1] >= '0' && buf[1] <= '9') ? (buf[1] - '0')&0x0f : 
			( (buf[1] >= 'a' && buf[1] <= 'f') ? (buf[1] - 'W')&0x0f : 
			  (buf[1] - '7')&0x0f );
	return ret;
}
...
int main(int argc, char *argv[])
{
	...
	while(fgets(buf,1024, FL)) {
		...
		for (i = 0; i< len; i += 2) {
			edid_buf[j] = string_to_hex(&buf[i]);
			printf("%.2x", edid_buf[j++]);
		}		
	}
	
	ParseEdidBlock0(edid_buf);
	ParseEdidBlock1(edid_buf + EDID_LENGTH);
